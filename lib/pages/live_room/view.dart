...
import 'package:PiliPro/pages/live_room/superchat/superchat_panel.dart';
import 'package:PiliPro/pages/live_room/widgets/bottom_control.dart';
import 'package:PiliPro/pages/live_room/widgets/chat_panel.dart';
import 'package:PiliPro/pages/live_room/widgets/header_control.dart';
...
class _LiveRoomPageState extends State<LiveRoomPage>
  ...
  void initState() {
    ...
    if (isReturningFromPip) {
      // 是从小窗返回，我们需要手动恢复弹幕和定时器
      // 因为 controller 的 onInit 已经跳过了播放器初始化
  ...
  Future<void> didPopNext() async {
    ...
    // 检查 controller 是否是从PiP返回的状态
    if (_liveRoomController.isReturningFromPip) {
      _liveRoomController.isReturningFromPip = false; // 重置标志
      _liveRoomController
        ..danmakuController?.resume()
        ..startLiveMsg();
      plPlayerController.addStatusLister(playerListener);
  ...
  void _startPipMode() {
    ...
    LivePipOverlayService.startLivePip(
      ...
      onReturn: () {
        ...
        Get.toNamed(
          '/liveRoom',
          arguments: _liveRoomController.roomId,
        );
      },
    );
  }
