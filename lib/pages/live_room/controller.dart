...
class LiveRoomController extends GetxController {
  ...
  void onInit() {
    ...
    if (!isReturningFromPip) {
      ...
      // 是从小窗返回，我们假设播放器已在播放
      // 但我们仍然需要 liveTime 和 isPortrait
      // 暂时从 plPlayerController 获取
      isPortrait.value = plPlayerController.isVertical;
      isLoaded.value = true;
      // 马上会从 view.initState 恢复弹幕和定时器
    }

    queryLiveInfoH5(); // 总是获取最新的房间信息
  ...
  Future<void>? playerInit({bool autoplay = true}) {
    ...
    // 如果是从小窗返回，播放器已经在播放，跳过 setDataSource
    if (isReturningFromPip) {
      // isReturningFromPip 会在下次 queryLiveUrl 时重置（因为控制器会重建）
      return null;
    }
  ...
  Future<void> queryLiveUrl() async {
    ...
    if (res.isSuccess) {
      ...
      await playerInit(); // playerInit 内部会检查 isReturningFromPip
      isLoaded.value = true;
    } else {
